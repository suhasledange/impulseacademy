...
const BlogCard = ({id,image,title,date}) => {
  return (
    <div className='shadow-xl border-2 border-gray-300 flex flex-col min-h-[23rem] w-[24rem] items-center p-8 rounded-2xl hover:shadow-xl duration-200'>
      
        <div className='relative w-full h-32 mb-10'>
                <img className='w-full h-full object-contain' src={image} alt='image'/>
                <div className='absolute rounded-full p-3 w-10 h-10 bg-text-head flex items-center justify-center text-white -bottom-6 right-10'>
                            {id}
                </div>
        </div>
